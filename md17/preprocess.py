...
for molecule_type in molecule_list:
    print(molecule_type)
    data_dir = "dataset/md17_" + molecule_type + ".npz"
    ...
    # Save the atomic number of each atom
    np.save("processed_dataset/" + molecule_type + "_z.npy", z)
    train_data = []
    test_data = []
    val_data = []
    ...
    for i in range(num_period_segment):
        ...
        x_period_val = x_period[
            train_period_length : train_period_length + val_period_length
        ]
        ...
        for j in range(num_train):
            traj = x_period_train[
                sample_frequency * j : sample_frequency * j + traj_length : framegap
            ]
